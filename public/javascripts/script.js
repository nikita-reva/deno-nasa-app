...
async function loadLaunches() {
	...
	try {
		const launchesResponse = await fetch('/launches');
		const fetchedLaunches = await launchesResponse.json();
		return (launches = fetchedLaunches.sort((a, b) => {
			return a.flightNumber < b.flightNumber;
		}));
	} catch (error) {
		console.log(error);
...
async function loadPlanets() {
	// return fetch('/planets', { mode: 'no-cors' })
	...
	try {
		const planetsResponse = await fetch('/planets', { mode: 'no-cors' });
...
async function submitLaunch() {
	...
	return (document.getElementById('launch-success').hidden = false);
}
